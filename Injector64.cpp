...
namespace Injector64 {
	...
	bool ManualMapDll(HANDLE hProc, BYTE* pSrcData, SIZE_T FileSize, bool ClearHeader, bool ClearNonNeededSections, bool AdjustProtections, bool SEHExceptionSupport, DWORD fdwReason, LPVOID lpReserved) {
		...
		if (pOldFileHeader->Machine != CURRENT_ARCH) { //检测dll的所属平台
		...
		if (!WriteProcessMemory(hProc, MappingDataAlloc, &data, sizeof(MANUAL_MAPPING_DATA), nullptr)) { //寫入映射
		...
		if (!WriteProcessMemory(hProc, pShellcode, Shellcode, 0x1000, nullptr)) { //寫入Shell Code
